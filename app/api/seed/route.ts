  ...
  try {
    ...
    if (!instructor) {
      try {
        ...
        console.error('Error creating instructor:', userError);
        return NextResponse.json(
          { success: false, error: 'Failed to create instructor user', details: String(userError) },
          { status: 500 }
        );
      }
    }
    ...
    try {
      ...
      return NextResponse.json({
        ...
        courses: createdCourses.map(c => ({ id: c._id, title: c.title.en, slug: c.slug })),
      });
    } catch (courseError) {
      console.error('Error creating courses:', courseError);
      return NextResponse.json(
        { success: false, error: 'Failed to create courses', details: String(courseError) },
        { status: 500 }
      );
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to seed courses', details: String(error) },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  return NextResponse.json({
    message: 'Use POST to seed the database with sample courses',
    coursesCount: sampleCourses.length,
    courses: sampleCourses.map(c => ({ title: c.title.en, slug: c.slug, price: c.price, currency: c.currency })),
  });
}
