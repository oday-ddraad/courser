...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      ...
      // Check if profile is completed - middleware will handle redirect
      // But we can also check here for better UX
  ...
  return (
    <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800">
      ...
      <button
        ...
        type="button"
        className="w-full py-3 px-4 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-200 font-semibold rounded-lg transition-all flex items-center justify-center gap-3"
      >
