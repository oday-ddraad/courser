        ...
        <div className="flex md:order-2 space-x-2 rtl:space-x-reverse items-center">
          ...
          <div className="hidden md:flex items-center space-x-2 rtl:space-x-reverse">
            {status === "authenticated" ? (
              <>
                <Link href="/dashboard" className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 hover:text-blue-600 transition-colors">
                  {t('dashboard')}
                </Link>
                <button 
                  onClick={() => signOut({ callbackUrl: '/' })}
                  className="px-4 py-2 text-sm font-medium text-white bg-red-500 rounded-lg shadow-md hover:bg-red-600 active:scale-95 transition-all"
                >
                  {t('logout')}
                </button>
              </>
